...
def munge(df1, df2, nope_df, sp_500_df, etf_df, spy_df):
       ...
       spy_df_today = spy_df[spy_df['Date'] == df2_date].head(1)
       spy_delta = (spy_df_today['Close'] - spy_df_today['Open']) / spy_df_today['Open']
       try:
              df2['spy_delta'] = spy_delta.item()
              df2['spy_delta'].round(4)
       except:
              # if there is a missing day in the SPY data
              spy_df.index = range(len(spy_df.index))
              df2_date = pd.to_datetime(df2_date)
              print (spy_df)
              print (df2_date, type(df2_date))
              yest = df2_date - dt.timedelta(days=1)

              yesterday_frame = spy_df[spy_df['Date'] == yest].head(1)
              yesterday_close = yesterday_frame['Close'].item()
              q = spy_df.index[spy_df['Date'] == yest]

              tomorrow_frame = spy_df.iloc[(q+1)]
              tomorrow_open = tomorrow_frame['Open'].item()

              delta = (tomorrow_open - yesterday_close) / yesterday_close
              print (yesterday_close, tomorrow_open, delta)
              df2['spy_delta'] = delta
              df2['spy_delta'].round(4)
              input('enter')
