...
before_nope_file = f'../nope_dataframes/2021-02-02_09.30_nope.csv'
nope_df = pd.read_csv(before_nope_file, compression = 'gzip')
nope_df['symbol'] = nope_df['ticker']
print (nope_df)

before_stock_file = f'../stock_dataframes/2021-02-02_09.30.csv'
before_stock_df = pd.read_csv(before_stock_file, compression = 'gzip')

after_stock_file = f'../stock_dataframes/2021-02-02_16.19.csv'
...
df1 = df1.sort_values(by=dep_var, ascending = False)
df1 = df1[~df1.isin([np.nan, np.inf, -np.inf]).any(1)]


#df1['weighted_investment'] = df1['nope_metric'].sum()
...
sns.regplot(df1[dep_var],df1['change_from_before'])
plt.show()
